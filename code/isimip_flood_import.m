...
%
%   Special: since flood footrpints are 'tiny', i.e. should not be
...
% CALLING SEQUENCE:
%   hazard=isimip_flood_import(flood_filename,hazard_filename,entity,check_plot)
% EXAMPLE:
%   entity=climada_entity_load('USA_UnitedStates_Florida');
%   hazard=isimip_flood_import('USA_UnitedStates_Florida_FL.nc','auto',entity);
%   hazard=isimip_flood_import('global_FL.nc','auto',entity);
% INPUTS:
%   flood_filename: filename of the .nc file with the flood
%       footprints, default folder is ..climada_data/isimip/
%       If empty, the default (TEST) file global_FL.nc in the data folder
...
%       If='' (empty), it is set to 'isimip_DEU_TEST_FL.mat';
%   country_ISO3: the ISO3 code of the country to generate the hazard set
%       for, (if empty, uses ='DEU' for tests)
...
if isempty(flood_filename) % to run TEST without arguments
    flood_filename=[module_data_dir filesep 'isimip' filesep 'global_FL.nc'];
    fprintf('NOTE: flood_filename is set to %s',flood_filename)
end

if isempty(hazard_filename) % to run TEST without arguments
    hazard_filename='isimip_DEU_TEST_FL';
    fprintf('NOTE: hazard_filename is set to %s',hazard_filename)
end
...
[nc_grid.X,nc_grid.Y] = meshgrid(nc_grid.lat,nc_grid.lon);

for i=1:length(nc_grid.lon)
    for j=1:length(nc_grid.lat)
        if nc_grid.NatId(i,j)>1
            plot(nc_grid.X(i,j),nc_grid.Y(i,j),'.r');
        end
    end
end
